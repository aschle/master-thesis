...
[TODO: x\% von den Mean der Detection Frequency ]
...
\item \textbf{Discard IDs below a cutoff}\\
For each ID its detection frequency is measured. All detections, whos ID frequency is below a certain cutoff are discarded. The cutoff is measured in percent of the average detection frequency.
...
\item[CutOff] This value is set to XXX, as explained in section~\ref{subsec:quality}.
...
\subsection{Remove faulty nodes}
nodes with a low closeness are removed\\
kmeans, gaussian mixture model\\
nodes with low closeness have a negative age\\
TODO: add PLOT with closeness and age and kmean groups/goussian groups\\
